...
namespace collision
{
    ...
    bool PixelPerfectTest(const sf::Sprite& Object1, const sf::Sprite& Object2, sf::Uint8 AlphaLimit) {
        ...
        if (Object1.getGlobalBounds().intersects(Object2.getGlobalBounds(), Intersection)) {
            ...
            for (int i = static_cast<int>( Intersection.left );
                 i < Intersection.left+Intersection.width;
                 i++)
                {
                for (   int j = static_cast<int>(Intersection.top);
                        j < Intersection.top+Intersection.height;
                        j++
                        )
                    {
    ...
    public:
        OrientedBoundingBox (const sf::Sprite& Object) // Calculate the four points of the OBB from a transformed (scaled, rotated...) sprite
        {
            ...
            Points[2] = trans.transformPoint(
                static_cast<float>(local.width),
                static_cast<float>(local.height)
                );
